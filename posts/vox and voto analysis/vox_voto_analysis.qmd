...
vox$annee_f <- as.factor(vox$annee)
```
...
```{r create coefficient dataframe}
#| echo: false
#| warning: false
#| message: false
...
#| warning: false
#| message: false
breaks_inc <- seq(min(ame_inc$ame), max = max(ame_inc$ame), by = 0.01)
ame_inc %>% 
  ggplot()+
  aes(x = annee, y = ame)+
  ...
  scale_y_continuous(breaks = breaks_inc,
                     labels = scales::percent_format())+
  labs(title = "Effect of income on the probability 
to vote for the Swiss Socialist Party",
...
breaks_educ <- seq(0, max = max(ame_educ$ame), by = 0.01)
ame_educ %>% 
  ggplot()+
  aes(x = annee, y = ame)+
  geom_hline(yintercept = 0, color = "red")+
  ...
  geom_line()+
  geom_pointrange(aes(ymin = low, ymax = high))+
  scale_y_continuous(breaks = breaks_educ,
                     labels = scales::percent_format())+
  labs(title = "Effect of education on the probability 
to vote for the Swiss Socialist Party",
       x = "Average Marginal Effect (% points)",y = "")
```
