...
tbl_class_parties %>% 
  group_by(parties) %>% 
  diverging_bar_chart(x = class, y = diff, text_size = 5)+
  facet_wrap(~parties)+
...
tbl_decile_parties %>% 
  group_by(parties) %>% 
  diverging_bar_chart(x = IncomeDecile, y = diff, text_size = 5)+
  facet_wrap(~parties)+
  scale_y_continuous(labels = scales::percent)+
...
From @fig-divbarcharts and @fig-decileplot, we can conclude that the SP is now above all supported by socio-cultural professionals and does not draw above average support from production workers.
