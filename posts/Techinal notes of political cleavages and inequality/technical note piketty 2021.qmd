...
This graph is very close to the one from the techinal note (figure one, right-sided graph)
...
```{r}
decile_vec <- function(data, columns, prop_vote, share_decile, share_rest){
...
decile_vec <- capture.output(for (i in min(columns):max(columns)) {
  ...
  decile =  c(weighted.mean(x = c(data$prop_vote[data[,i] == 1]),
...
```{r}
decile_vec(data = table.income, columns = 11:20, prop_vote = table.income$prop_vote, share_decile = table.income$share_decile, share_rest = table.income$share_rest)
```
