...
load("F:/myblog/posts/Techinal notes of political cleavages and inequality/decile.rda")
load("F:/myblog/posts/Techinal notes of political cleavages and inequality/weightshare.rda")
load("F:/myblog/posts/Techinal notes of political cleavages and inequality/deciledummies.rda")
library(tidyverse)
library(nnet)
library(haven)
library(sjlabelled)
...
datareg <- panel2019 %>% dplyr::select("vote.left", "education.level", "Education.level", "Gross.monthly.hh.income", "religion", "gender", "W1_f10000", "W1_f20900", "W1_age")
...
table_income <- datareg %>% 
  filter(vote.left == 1) %>% 
  group_by(Gross.monthly.hh.income) %>% 
  count(vote.left) %>% 
  drop_na() %>% 
  ungroup() %>% 
  mutate(prop_vote = table_incomeprop$prop[table_incomeprop$vote.left == 1],
         prop = n/sum(n),
         rangeleft = lag(cumsum(n/sum(n)), default = 0),
         rangeright = cumsum(n/sum(n)))
...
```{r}
#| include: false
table_income$d_share <- weight_share(x = table_income$rangeright)
table_income$r_share <- 1-weight_share(x = table_income$rangeright)
```

```{r}
#| include: false
table_income <- decile_dummies(data = table_income, rangeleft = rangeleft, rangeright = rangeright)
```

```{r}
#| include: false
decile <- decile(data = table_income, columns = 10:19, prop = table_income$prop_vote, share_decile = table_income$d_share, share_rest = table_income$r_share)
```

```{r}
#| include: false
datareg <- datareg %>% mutate(id = 1:nrow(datareg))
```

```{r}
#| include: false
join <- datareg %>% left_join(table_income, by = "Gross.monthly.hh.income")
```

```{r}
#| include: false
join[rep(1:nrow(join), 10),] -> dataregexpand
...
table_income %>% 
  ggplot()+
  aes(x = factor(Gross.monthly.hh.income), y = prop_vote)+
  geom_col()
```
...
The estimated income coefficient is even closer to zero after controls, suggesting an even lower link between income group and the probability to vote for the left. However, the coefficient for education level has increased. Age is very close to 0 while non religiosity is linked positively. The coefficient for gender implies that, ceteris paribus, women vote on probabiliy 0.43 more than men for the left.
It would be interesting to see how the model changes if an interaction term between education and income is included. This interaction would allow the the slope for education and income to vary according to each other's values:
...
### Graphical representation of this more complex model
