...
############################################################
####dummies for unemployed and retired ############
...
datareg <- panel2019 %>% dplyr::select("vote_left", "vote_plr", "vote_greens", "vote_greens_lib", "vote_udc", "vote_liberal", "income", "education", "age", "nonreligious", "unempl", "retired", "public", "full_time", "excutive_superviser_selfemployed", "married", "against_immigr_limit", "against_eu_agreements", "against_eu_membership", "state_or_freemarket", "against_eq_opport_foreigners", "against_taxes_high_income", "for_social_spending", "foreigners_share", "W1_3_canton_sample")
...
reg2 <- glm(data = datareg, vote_left ~ education + income + age + nonreligious + full_time + public + married, family=binomial(link="logit"))

reg3 <- glm(data = datareg, vote_left ~ education + income + age + nonreligious + full_time + public + married + against_immigr_limit + against_eu_membership + state_or_freemarket, family=binomial(link="logit"))

regudc <- glm(data = datareg, vote_udc ~ education + income + age + nonreligious + full_time + public + married + against_immigr_limit + against_eu_membership + state_or_freemarket, family=binomial(link="logit"))

regplr <- glm(data = datareg, vote_plr ~ education + income + age + nonreligious + full_time + public + married + against_immigr_limit + against_eu_membership + state_or_freemarket, family=binomial(link="logit"))

reggreen <- glm(data = datareg, vote_greens ~ education + income + age + nonreligious + full_time + public + married + against_immigr_limit + against_eu_membership + state_or_freemarket, family=binomial(link="logit"))

reggreenlib <- glm(data = datareg, vote_greens_lib ~ education + income + age + nonreligious + full_time + public + married + against_immigr_limit + against_eu_membership + state_or_freemarket, family=binomial(link="logit"))
```
...
stargazer(reg, reg2, reg3, regudc, regplr, reggreen, reggreenlib, type = "html", title = "Vote for political parties: log(odds)")
```
