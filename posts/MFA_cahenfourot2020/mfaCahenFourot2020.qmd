...
data_dim = MFA_CF2020$separate.analyses$Environment$ind$coord |> 
  as.data.frame() |> 
  rownames_to_column()
...
varplot = function(...){lapply(axes_list, function(a) fviz_pca_var(MFA_CF2020$separate.analyses$Environment, axes = a))}
plots <- lapply(axes_list, function(a) fviz_pca_var(MFA_CF2020$separate.analyses$Environment, axes = a))



ggpairs(data_dim, columns = 2:6,
        lower = list(continuous = lapply(axes_list, function(a) fviz_pca_var(MFA_CF2020$separate.analyses$Environment, axes = a))),
        diag = NULL, upper = list(continuous = "points"),
        mapping = aes(color = rowname))


library(factoextra)
fviz_pca_var(MFA_CF2020$separate.analyses$Environment) # graph of the variables and their contribution to the dimensions
fviz_pca_ind(MFA_CF2020$separate.analyses$Environment,
             axes = c(1,2)) # graph of individuals (here countries)
```
...
# Extraction des coordonnées des individus sur les 5 premiers axes
coords <- as.data.frame(MFA_CF2020$ind$contrib[, 1:4])
MFA_CF2020$ind$contrib
# Pairs plot avec base R
pairs(coords, main="Pairs Plot des 5 premières dimensions de la MFA")

# Pairs plot avec GGally (ggplot2)
ggpairs(coords,
        upper = list(continuous = wrap("points")),  # Scatter plots en haut
        lower = list(continuous = wrap("points")),  # Scatter plots en bas
