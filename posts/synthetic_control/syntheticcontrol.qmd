...
I will now show how the synthetic control method can be run in R by comparing three different packages `Synth`, `tidysynth` and `synthdid`. I will reproduce @abadie_comparative_2015 's results on the negative effects of German reunification on West Germany's economic growth.
...
library(sjlabelled)
library(gt)
setwd("~/GitHub/celalguney/posts/synthetic_control")
...
results_bsynth <- bayesianSynth$new(data = data,
                                   time = year,
                                   id = country,
                                   treated = treatment,
                                   outcome = gdp,
...
```{r}
results_bsynth$timeTiles + ggplot2::xlab("Year") + ggplot2::ylab("Country")
```



```{r}
results_bsynth$fit(cores = 4)

# Vizualize the Bayesian Synthetic Control
results_bsynth$synthetic + 
  ggplot2::xlab("Year") +
...
```{r}
results_bsynth$weightDraws()
results_bsynth$effectPlot(facet = TRUE, subset = NULL)
```
